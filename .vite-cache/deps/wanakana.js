...
function makeOnInput(options) {
	let prevInput;
	const mergedConfig = Object.assign({}, mergeWithDefaultOptions(options), { IMEMode: options.IMEMode || true });
...
function convertInput(target, options, map, triggers, prevInput) {
	...
	if (textToConvert !== convertedText) {
		...
		if (tail.length) setTimeout(() => target.setSelectionRange(newCursor, newCursor), 1);
		else target.setSelectionRange(newCursor, newCursor);
	} else target.value;
}
function onComposition({ type, target, data }) {
	if (/Mac/.test(window.navigator && window.navigator.platform)) {
		if (type === "compositionupdate" && isJapanese(data)) target.dataset.ignoreComposition = "true";
		if (type === "compositionend") target.dataset.ignoreComposition = "false";
	}
}
...
var onInput = ({ target: { value, selectionStart, selectionEnd } }) => console.log("input:", {
	value,
	selectionStart,
	selectionEnd
});
...
function toKatakana(input = "", options = {}) {
	...
	if (isMixed(input) || isRomaji(input) || isCharEnglishPunctuation(input)) return hiraganaToKatakana(toKana(input.toLowerCase(), mergedOptions));
