...
function envWithLocalBin(env) {
  ...
  for (const key of ["PATH", "Path"]) {
    if (typeof next[key] === "string" && next[key].length > 0) {
      next[key] = `${bin}${delimiter}${next[key]}`;
    }
  }
  if (!next.PATH && !next.Path) next.PATH = bin;
  return next;
}

/** Resolve `vite` to its JS entry so Windows/Git Bash can spawn it without .cmd. */
function resolveSpawn(command, args) {
  if (command === "vite") {
    const viteJs = join(projectRoot(), "node_modules", "vite", "bin", "vite.js");
    if (existsSync(viteJs)) {
      return { cmd: process.execPath, args: [viteJs, ...args], shell: false };
    }
  }
  ...
  if (existsSync(localBin)) {
    return { cmd: localBin, args, shell: isWin };
  }
  return { cmd: command, args, shell: isWin };
}
